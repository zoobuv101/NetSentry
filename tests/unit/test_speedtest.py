...
class TestSpeedTestRunner:
    ...
    async def test_ookla_returns_result(self) -> None:
        """Ookla runner parses JSON output correctly."""
        ...
        fake_output = """{
            ...
            "ping": {"latency": 12.5},
            "server": {"name": "Speedtest Server", "location": "London"}
        }"""
        ...
        assert result is not None
        assert abs(result.download_mbps - 80.0) < 1.0  # 10000000 bytes/s → ~80 Mbps
